...
class OwnerFilterScreen extends StatefulWidget {
  ...
  @override
  State<OwnerFilterScreen> createState() => _OwnerFilterScreenState();
}

class _OwnerFilterScreenState extends State<OwnerFilterScreen>
    with AppBarMixin {
  ...
  @override
  void initState() {
    context.read<OwnerFilterCubit>().getData(context);
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<OwnerFilterCubit, OwnerFilterState>(
      listener: (context, state) {
        ...
        if (state is FilterStatusFailure ||
            state is FilterCurrencyFailure ||
          ...
          } else if (state is FilterCurrencyFailure) {
            errorMessage = state.errorMessage;
          } else if (state is FilterCountryFailure) {
            errorMessage = state.errorMessage;
          } else if (state is FilterCityFailure) {
          ...
          bottomNavigationBar: ButtonRow(
            ...
            onRightButtonTap: () async {
              if (validateFilters(context)) {
                ...
                context.read<OwnerFilterCubit>().applyFilter(context);
  ...
  Widget citySelectionFilterComponent(BuildContext context) {
    OwnerFilterCubit cubit = context.read<OwnerFilterCubit>();
    return BlocListener<OwnerFilterCubit, OwnerFilterState>(
      listener: (context, state) {},
  ...
  void buildBlocListener(BuildContext context, OwnerFilterState state) {}
          ...
          children: [
            ...
            Text(
              localizedDistanceText(
                  context, context.read<OwnerFilterCubit>().filterRadiusValue),
              style: Theme.of(context)
                  .textTheme
                  .titleSmall
              ...
              child: SfSliderTheme(
                ...
                child: Padding(
                  ...
                  child: SfSlider(
                    min: 0,
                    max: 100,
                    value: context.read<OwnerFilterCubit>().filterRadiusValue,
                    interval: 20,
                    stepSize: 20,
                    showDividers: false,
                    showLabels: true,
                    activeColor: AppColors.colorPrimary,
                    inactiveColor: AppColors.colorPrimaryShade,
                    enableTooltip: false,
                    thumbIcon: Container(

...
part 'in_review_filter_state.dart';

class InReviewFilterCubit extends Cubit<InReviewFilterState> {
  InReviewFilterCubit(
      {required this.filterRepository, required this.propertyRepository})
      : super(InReviewFilterInitial());
  ...
  bool get isFilterApplied => state is ApplyInReviewFilter;
  ...
  Future<void> getFilterStatusList(BuildContext context) async {
    emit(InReviewFilterStatusLoading());
    ...
    if (response is SuccessResponse &&
      ...
      emit(InReviewFilterStatusSuccess(dataList: responseModel.data ?? []));
    } else if (response is FailedResponse) {
      emit(InReviewFilterStatusFailure(response.errorMessage));
    }
  }

  /// furnished status API
  Future<void> getFurnishedStatus() async {
    emit(PropertyFurnishedLoading());

    final response = await propertyRepository.getFurnishedType();
    ...
    if (response is SuccessResponse &&
      ...
      furnishedStatusList = furnishedStatusResponseModel.data ?? [];

      emit(PropertyFurnishedSuccess());
    } else if (response is FailedResponse) {
      emit(PropertyFurnishedFailure(response.errorMessage));
    }
  }
  ...
  void clearFilters(BuildContext context) {
    ...
    emit(InReviewFilterReset());
    emit(InReviewFilterInitial());
  }
  ...
  Future<void> applyFilter(BuildContext context) async {
    ...
    if (filterRequestModel.isEmpty()) {
      ...
      emit(ApplyInReviewFilter(filterRequestModel));
    }
  }
}
